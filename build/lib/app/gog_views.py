...
def dashboard():
    # Count unread messages from admins for this user
    unread_count = 0
    ...
    return render_template("gog/gog_dashboard.html", unread_count=unread_count)
...
def messages():
    ...
    unread_admin_messages = Message.query.filter_by(
        conversation_id=conversation.id,
        is_from_admin=True,
        is_read=False
    ).all()
